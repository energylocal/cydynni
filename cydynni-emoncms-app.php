...
function show() 
{
    ...
    update();
		...
		updaterinst = setInterval(update,60000);
...
function load() {
    ...
    if ((((new Date()).getTime()-view.end)<3600*1000*48) && ((view.end-lasttime)*0.001)>1800) {
        ...
        // ----------------------------------------------------------------------------
        $.ajax({                                      
            url: path+"cydynni/hydro-estimate?start="+view.start+"&end="+view.end+"&interval="+interval+"&lasttime="+lasttime+"&lastvalue="+lastvalue,
        ...
        $.ajax({                                      
            url: path+"cydynni/community-estimate?lasttime="+lasttime+"&interval="+interval,
            dataType: 'json',
            async: false,                      
            success: function(result) {
