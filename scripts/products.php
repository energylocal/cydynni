...
function get_current_tariff($tariff_history) {
	if (count($tariff_history) < 1) {
            throw new Exception('No tarrifs in history');
	}

	$now = time();
	foreach ($tariff_history as &$tarrif) {
		if ($tarrif['start'] < $now && $now < $tarrif['end']) {
			return $tarrif;
		}
print "no";
	}
	throw new Exception('Could not establish current tarrif');
}
...
foreach ($club_settings as $key=>$club) {
    $tarrif = get_current_tariff($club['tariff_history']);
    print($key);
    print_r($tarrif);
